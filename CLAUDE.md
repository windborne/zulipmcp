...
- **`auto_typing_start=False` gates starts only; `listen()`/`end_session()` still send "stop".** Zulip keys the indicator per user, not per process, so gating stops can't isolate an external owner — it would only leave a stale bar on idle/exit. Don't gate the stops.
