...
- **`auto_typing_start=False` gates starts only; `listen()`/`end_session()` still send "stop", and `reply()` sends "stop" in place of its start.** Zulip keys the indicator per user, not per process, so gating stops can't isolate an external owner — it would only leave a stale bar on idle/exit, and a reply with no stop leaves the pre-reply status stranded under the new message until the server expiry. Don't gate the stops. `set_context()` stays a no-op there: a stop that early would wipe whatever the owner posted at boot.
