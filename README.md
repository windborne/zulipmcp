...
An `ear_with_hearing_aid` emoji is added to the last message as a visual indicator while listening and removed when listening stops. If the realm has a custom emoji named `robot_ear`, that is used instead (checked once per process, no per-call overhead). MCP keepalive pings are sent via `ctx.info()` after each long-poll cycle.
