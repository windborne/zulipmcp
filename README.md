...
A `robot_ear` emoji is added to the last message as a visual indicator while listening and removed when listening stops. This is a custom emoji — if your Zulip realm doesn't have one named `robot_ear`, the indicator is silently skipped and listening works normally. To enable it, [upload a custom emoji](https://zulip.com/help/custom-emoji) named `robot_ear` in your Zulip organization settings. MCP keepalive pings are sent via `ctx.info()` after each long-poll cycle.
